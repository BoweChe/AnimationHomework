...
void Bone_Animation::init()
{
    ...
    angles[6] = yellowXangle;
    angles[3] = yellowYangle;
    angles[0] = yellowZangle;
    
    angles[7] = purpleXangle;
    angles[4] = purpleYangle;
    angles[1] = purpleZangle;
    
    angles[8] = blueXangle;
    angles[5] = blueYangle;
    angles[2] = blueZangle;
    
    //forward_kinematics(yellowXangle,yellowYangle,yellowZangle,purpleXangle,purpleYangle,purpleZangle,blueXangle,blueYangle,blueZangle);
    /*
...
float Bone_Animation::calculate_beta(){
    float beta;
    Eigen::MatrixXf test1 = jacobian*get_transpose(0);
    //glm::vec3 test2 = test1*goalVector;
    //std::cout << test1(0,2) << std::endl;
    //std::cout << sizeof(test1)/sizeof(test1[0]) << std::endl;
    glm::vec3 lower_val = glm::vec3(0.0f);
    for(int i=0;i<3;i++){
        lower_val[i] = test1(i,0)*goalVector(i,0)+test1(i,1)*goalVector(i,0)+test1(i,2)*goalVector(i,0);
    }
    
    float lower_beta = sqrt(lower_val[0]*lower_val[0]+lower_val[1]+lower_val[1]+lower_val[2]*lower_val[2]);
    
    float higher_beta[9] = {};
    for(int i=0;i<9;i++){
        higher_beta[i] = get_transpose(0)(i,0)*goalVector(0,0)+get_transpose(0)(i,1)*goalVector(1,0)+get_transpose(0)(i,2)*goalVector(2,0);
    }
    float high_beta = 0.0f;
    for(int i=0;i<9;i++){
        high_beta+=higher_beta[i]*higher_beta[i];
    }
    high_beta = sqrt(high_beta);
    
    beta = high_beta/lower_beta;
    std::cout << beta << std::endl;
    if(beta>1.0f||isnan(beta)){
        beta = 0.1f;
    }
    ...
    float angleBlueZ)
{
    ...
    glm::vec3 pivot = glm::vec3(0.0f, 2.5f, 0.0f);
    ...
    glm::vec3 pivot2 = glm::vec3(0.0f, 6.0f, 0.0f);
    ...
    glm::vec3 pivot3 = glm::vec3(0.0f, 8.5f, 0.0f);
