...
//public degree;
//glm::vec2 degree = glm::vec2(1.0f, 10.0f);
float xdegree = 30.0f;
float ydegree = 20.0f;
float realx = 0.0f;
float realy = 0.0f;
...
void Renderer::nanogui_init(GLFWwindow* window)
{
    ...
    //glm::vec2 degree = glm::vec2(1.0f, 10.0f);
    //int i=2;
    gui_1->addVariable("Rotate Angle x", xdegree)->setSpinnable(true);
    gui_1->addVariable("Rotate Angle y", ydegree)->setSpinnable(true);
  
    gui_1->addButton("Rotate Local X",[](){
        realx = realx+xdegree;
        //xdegree = xdegree/(m1-1);
        //xdegree = xdegree*m1;
        //m1 = m1+1;
        //k=1;
    });
 
    gui_1->addButton("Rotate Local Y",[](){
        //m_camera->reset();
        realy = realy+ydegree;
...
void Renderer::init()
{
	...
	this->m_window = glfwCreateWindow(m_camera->width, m_camera->height, "asst1", nullptr, nullptr);
...
void Renderer::draw_scene(Shader& shader)
{
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		...
		if (obj_list[i].obj_name == "main_object")
		{
            ...
            main_object_model_mat = glm::translate(main_object_model_mat,glm::vec3(-5.0f,0.0f,0.0f));
            main_object_model_mat = glm::rotate(main_object_model_mat,glm::radians(realy), glm::vec3(0.0f,1.0f,0.0f));
            main_object_model_mat = glm::translate(main_object_model_mat,glm::vec3(5.0f,0.0f,0.0f));
            main_object_model_mat = glm::rotate(main_object_model_mat,glm::radians(realx), glm::vec3(-1.0f,0.0f,0.0f));
