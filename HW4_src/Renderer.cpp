...
float ft1 = Renderer::m_aircraft_animation->t1;
float ft2 = Renderer::m_aircraft_animation->t2;
float fulltime = Renderer::m_aircraft_animation->total_moving_time;
bool cval = false;
bool aval = false;
bool eval = true;
//float realx = 45.0f;
void Renderer::nanogui_init(GLFWwindow* window)
{
    ...
    gui_1->addVariable("Catmull-Rom curve on/off",cval);
    gui_1->addVariable("Rotate on/off",eval);
    gui_1->addGroup("aircraft Animation");
    gui_1->addVariable("aircraft is moving",aval);
    gui_1->addVariable("t1",ft1);
    gui_1->addVariable("t2",ft2);
    gui_1->addVariable("total moving time",fulltime);
    gui_1->addButton("Reset Movement",[](){
        m_aircraft_animation->clear_table();
        m_aircraft_animation->t1 = ft1;
        m_aircraft_animation->t2 = ft2;
        m_aircraft_animation->total_moving_time = fulltime;
        m_aircraft_animation->make_table();
...
void Renderer::load_models()
{
	...
	Object aircraft_object("../src/objs/aircraft.obj");
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		if (obj_list[i].obj_name == "cube")
		{
			...
			for (unsigned int j = 0; j < m_curve->control_points_pos.size(); j++) {
                ...
                cur_obj_model_mat = m_curve->calculate_quaternion(cur_obj_model_mat,j);
                
                
                
				cur_obj_model_mat = glm::scale(cur_obj_model_mat, glm::vec3(0.5, 0.5, 0.5));
		...
		if (obj_list[i].obj_name == "aircraft")
		{
            ...
            if(eval==true){
                cur_aircraft_model_mat = m_aircraft_animation->getAirquaternion(cur_aircraft_model_mat);
            }
            //cur_aircraft_model_mat = m_aircraft_animation->getAirquaternion(cur_aircraft_model_mat);
            //cur_aircraft_model_mat = m_aircraft_animation->get_model_mat();
